...
from .local_gobal_att import ASEM, DSEM
...
class Generator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.Vmamba_Encoder = Backbone_VSSM()

        self.fusion_net = ASEM(channels=512)
        self.atten_G_L = DSEM(in_channels=512)
    ...
    def compute_style(self, ST):
        ...
        FEAT_ST = self.Feat_Encoder(ST.view(B * N, 1, R, C))
        FEAT_ST = FEAT_ST.view(B, 512, 1, -1) # 8, 512, 1, 90
        # FEAT_ST_ENC = FEAT_ST.flatten(2).permute(2, 0, 1) # 90, 8, 512
        FEAT_ST_ENC = FEAT_ST.flatten(2).permute(0, 2, 1) # 8, 90, 512

        Feat_st_gobal = self.Vmamba_Encoder(ST.view(B * N, 1, R, C)) # 120, 512, 1, 6
        Feat_st_gobal = Feat_st_gobal.view(B, 512, 1, -1) # 8, 512, 1, 90
        # Feat_st_gobal_ENC = Feat_st_gobal.flatten(2).permute(2, 0, 1) # 90, 8, 512
        Feat_st_gobal_ENC = Feat_st_gobal.flatten(2).permute(0, 2, 1) # 8, 90, 512

        # Dual Aspect Attention (DAA)
        Att_Feat_st_g_l = self.atten_G_L( torch.add(FEAT_ST_ENC, Feat_st_gobal_ENC) ) # 8, 90, 512

        # Attentional Style Entanglement(ASE)
        G_L_Feat_ST = self.fusion_net(FEAT_ST_ENC.permute(0, 2, 1),
                                      FEAT_ST_ENC.permute(0, 2, 1) + Feat_st_gobal_ENC.permute(0, 2, 1)) # 8, 512, 90


        memory = self.encoder(Att_Feat_st_g_l.permute(1, 0, 2) + G_L_Feat_ST.permute(2, 0, 1)) # # 90, 8, 512
...
class GL_GAN(nn.Module):

    def __init__(self, args):
        super(GL_GAN, self).__init__()
        self.args = args
    ...
    def save_images_for_fid_calculation_ym(self, path, loader, split='train', allImg = False, realImg = False):
        if not isinstance(path, Path):
            path = Path(path)
        path.mkdir(exist_ok=True, parents=True)
        ...
        for step, data in enumerate(loader):
            ...
            for i, (fake, real, wid, lb, img_id) in enumerate(zip(fake_images, real_images, writer_ids, data['label'], data['idx'])):
                ...
                if allImg:
                    # img_id = '{:03d}_{:05d}.png'.format(wid, step + i)
                    img_id = '{:03d}_{:05d}.png'.format(wid, counter)
                    real_img_path = self.real_base / img_id
                    fake_img_path = self.fake_base / img_id
                else:
                    img_id = '{:05d}.png'.format(img_id)
                    real_img_path = self.real_base / "{:03d}".format(wid) / img_id
                    fake_img_path = self.fake_base / "{:03d}".format(wid) / img_id
                if realImg:
                    real_img_path.parent.mkdir(exist_ok=True, parents=True)
                    cv2.imwrite(str(real_img_path), 255 * real.squeeze())
    ...
    def save_images_for_fid_calculation_V(self, path, loader, split='train', allImg=False, realImg=False):
        ...
        for step, data in enumerate(loader):
            ...
            writer_ids = data['wcl'].int().tolist()
            """
            real image 是从data中拿出的真实图像，为iam数据集
            """
            for i, (fake, real, wid, lb, img_id) in enumerate(zip(fake_images, real_images, writer_ids, data['label'], data['idx'])):
                lb = lb.decode()
                ann[step * self.args.batch_size + i] = lb
